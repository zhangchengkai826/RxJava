...
As for RxJava, it doesn't have lots of demandings for hardware, but for softwares, it need to implement a lot of libraries. The following shows the required dependencies that needed for RxJava. But actually, there is a lot of hidden dependencies. While users download RxJava we don not need to download it specially, but Rxjava is developed on the base of it. We show it in Table 3.
```
...
|:----------------:|:------:|
|Java|Language that used for RXJava that can be written.|
...
9. What the difference in applying gradle plugin
